...
impl CsrfTokenManager {
    ...
    pub fn validate_logout_token(&self, received_token: &str) -> bool {
        let expected = self.logout_token().read_untracked();
        let is_valid = expected.as_str() == received_token;
        if is_valid {
            tracing::trace!("Validation of the logout CSRF token succeeded.");
        } else {
            tracing::warn!(
                expected_token = expected.as_str(),
                received_token,
                "Logout CSRF token validation failed. User was logged out via potential CSRF attack."
            );
        }
        is_valid
    }
...
mod tests {
    ...
    fn validate_correct_logout_token() {
        let manager = cut();
        assert_that(manager.validate_logout_token(manager.logout_token().get_untracked().as_str()))
            .is_true();
    }
    ...
    fn validate_unknown_logout_token() {
        let manager = cut();
        assert_that(manager.validate_logout_token("some_unknown_token")).is_false();
    }
    ...
    fn regenerate_creates_new_logout_token() {
        ...
        assert_that(manager.validate_logout_token(initial_token.as_str())).is_true();
        manager.regenerate();
        let new_token = manager.logout_token().get_untracked();
        assert_that(manager.validate_logout_token(initial_token.as_str())).is_false();
        assert_that(manager.validate_logout_token(new_token.as_str())).is_true();
