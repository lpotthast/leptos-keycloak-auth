...
pub struct OidcConfig {
    ...
    #[serde(flatten)]
    pub(crate) session_claims: OidcDiscoverySessionDiscoveryClaims,

    #[serde(flatten)]
    pub(crate) front_channel_logout_claims: OpenIDConnectFrontChannelLogoutDiscoveryClaims,

    #[serde(flatten)]
    pub(crate) back_channel_logout_claims: OpenIDConnectBackChannelLogoutDiscoveryClaims,

    #[serde(flatten)]
    pub(crate) oauth_claims: OAuthDiscoveryClaims,

    #[serde(flatten)]
    pub(crate) jarm_claims: OpenIDConnectJARMDiscoveryClaims,

    #[serde(flatten)]
    pub(crate) rp_initialized_claims: OpenIDConnectRPInitiatedLogoutClaims,
    ...
    // Note: Moved here, as it would otherwise need to be defined in both
    // `OpenIDConnectFrontChannelLogoutDiscoveryClaims` and
    // `OpenIDConnectBackChannelLogoutDiscoveryClaims`.
    pub(crate) sid: Option<String>,

    /// Contains all the additional fields not otherwise parsable.
    #[serde(flatten)]
    pub(crate) additional_claims: HashMap<String, serde_json::Value>,
}

/// See: <https://www.rfc-editor.org/rfc/rfc8414.html#section-2>
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[allow(clippy::struct_field_names)] // Allow all field names to start with "introspection_".
...
pub struct OpenIDConnectFrontChannelLogoutDiscoveryClaims {
    ...
    pub(crate) frontchannel_logout_supported: Option<bool>,

    /// OPTIONAL. Boolean value specifying whether the OP can pass iss (issuer) and sid (session ID)
    /// query parameters to identify the RP session with the OP when the `frontchannel_logout_uri`
    /// is used.
    /// If supported, the sid Claim is also included in ID Tokens issued by the OP.
    /// If omitted, the default value is false.
    /// The sid (session ID) Claim used in ID Tokens and as a `frontchannel_logout_uri` parameter
    /// has the following definition:
    pub(crate) frontchannel_logout_session_supported: Option<bool>,
    // Omitted the `sid` field. It would have to be defined in multiple places, so we simply added
    // it to `OidcConfig` directly.
}
...
pub struct OpenIDConnectBackChannelLogoutDiscoveryClaims {
    ...
    /// OPTIONAL. Boolean value specifying whether the OP can pass a sid (session ID) Claim in the
    /// Logout Token to identify the RP session with the OP.
    /// If supported, the sid Claim is also included in ID Tokens issued by the OP.
    /// If omitted, the default value is false.
    pub(crate) backchannel_logout_session_supported: Option<bool>,
    // Omitted the `sid` field. It would have to be defined in multiple places, so we simply added
    // it to `OidcConfig` directly.
}
...
pub struct OpenIDConnectRPInitiatedLogoutClaims {
    /// REQUIRED. URL at the OP to which an RP can perform a redirect to request that the End-User
    /// be logged out at the OP.
    /// This URL MUST use the https scheme and MAY contain port, path, and query parameter
    /// components.
    // Note: Required, but we do not know if the extension is even used...
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[allow(clippy::struct_field_names)] // Allow all field names starting with "authorization_".
pub struct OpenIDConnectJARMDiscoveryClaims {
    /// OPTIONAL. A JSON array containing a list of the JWS [RFC7515] signing algorithms
    /// (alg values) supported by the authorization endpoint to sign the response.
    pub(crate) authorization_signing_alg_values_supported: Option<Vec<String>>,

    /// OPTIONAL. A JSON array containing a list of the JWE [RFC7516] encryption algorithms
    /// (alg values) supported by the authorization endpoint to encrypt the response.
    pub(crate) authorization_encryption_alg_values_supported: Option<Vec<String>>,

    /// OPTIONAL. A JSON array containing a list of the JWE [RFC7516] encryption algorithms
    /// (enc values) supported by the authorization endpoint to encrypt the response.
    pub(crate) authorization_encryption_enc_values_supported: Option<Vec<String>>,
}

/// See: <https://openid.net/specs/openid-connect-session-1_0.html#OPMetadata>
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct OidcDiscoverySessionDiscoveryClaims {
    /// REQUIRED. URL of an OP iframe that supports cross-origin communications for session state
    /// information with the RP Client, using the HTML5 postMessage API.
    /// This URL MUST use the https scheme and MAY contain port, path, and query parameter
    /// components.
    /// The page is loaded from an invisible iframe embedded in an RP page so that it can run in
    /// the OP's security context.
    /// It accepts postMessage requests from the relevant RP iframe and uses postMessage to post
    /// back the login status of the End-User at the OP.
    // Note: Required, but we do not know if the extension is even used...
    pub(crate) check_session_iframe: Option<String>,
}
...
pub struct OpenIDConnectStandardDiscoveryClaims {
    ...
    /// If Issuer discovery is supported (see Section 2), this value MUST be identical to the
    ...
    /// The JWK Set MAY also contain the Server's encryption key(s), which are used by RPs to
    /// encrypt requests to the Server.
    /// When both signing and encryption keys are made available, a use (Key Use) parameter value
    /// is REQUIRED for all keys in the referenced JWK Set to indicate each key's intended usage.
    ...
    /// is used, although those defined in [OpenID.Core] SHOULD be listed, if supported.
    ...
    /// OP supports.
    /// Dynamic `OpenID Connect` providers MUST support the `code`, `id_token`, and the
    ...
    /// [OAuth.Responses].
    /// If omitted, the default for Dynamic `OpenID Connect` providers is `["query", "fragment"]`.
    ...
    /// supports.
    /// Dynamic `OpenID Connect` providers MUST support the `authorization_code` and `implicit`
    ...
    /// this OP supports.
    pub(crate) acr_values_supported: Option<Vec<String>>,
    ...
    /// supported by the OP for the ID Token to encode the Claims in a JWT [JWT].
    pub(crate) id_token_encryption_alg_values_supported: Option<Vec<String>>,

    /// OPTIONAL. JSON array containing a list of the JWE encryption algorithms (enc values)
    /// supported by the OP for the ID Token to encode the Claims in a JWT [JWT].
    pub(crate) id_token_encryption_enc_values_supported: Option<Vec<String>>,

    /// OPTIONAL. JSON array containing a list of the JWS [JWS] signing algorithms
    /// (alg values) [JWA] supported by the `UserInfo` Endpoint to encode the Claims in a JWT [JWT].
    /// The value none MAY be included.
    pub(crate) userinfo_signing_alg_values_supported: Option<Vec<String>>,

    /// OPTIONAL. JSON array containing a list of the JWE [JWE] encryption algorithms
    /// (alg values) [JWA] supported by the `UserInfo` Endpoint to encode the Claims in a JWT [JWT].
    pub(crate) userinfo_encryption_alg_values_supported: Option<Vec<String>>,

    /// OPTIONAL. JSON array containing a list of the JWE encryption algorithms
    /// (enc values) [JWA] supported by the `UserInfo` Endpoint to encode the Claims in a JWT [JWT].
    pub(crate) userinfo_encryption_enc_values_supported: Option<Vec<String>>,

    /// OPTIONAL. JSON array containing a list of the JWS signing algorithms
    /// (alg values) supported by the OP for Request Objects, which are described in Section 6.1
    /// of `OpenID Connect Core 1.0` [OpenID.Core].
    /// These algorithms are used both when the Request Object is passed by value
    /// (using the request parameter) and when it is passed
    /// by reference (using the `request_uri` parameter).
    /// Servers SHOULD support none and RS256.
    pub(crate) request_object_signing_alg_values_supported: Option<Vec<String>>,

    /// OPTIONAL. JSON array containing a list of the JWE encryption algorithms (alg values)
    /// supported by the OP for Request Objects.
    /// These algorithms are used both when the Request Object is passed by value and when it is
    /// passed by reference.
    pub(crate) request_object_encryption_alg_values_supported: Option<Vec<String>>,

    /// OPTIONAL. JSON array containing a list of the JWE encryption algorithms (enc values)
    /// supported by the OP for Request Objects.
    /// These algorithms are used both when the Request Object is passed by value and when it is
    /// passed by reference.
    pub(crate) request_object_encryption_enc_values_supported: Option<Vec<String>>,
    ...
    /// `private_key_jwt`, as described in Section 9 of `OpenID Connect Core 1.0` [OpenID.Core].
    ...
    /// by the Token Endpoint for the signature on the JWT [JWT]
    ...
    /// `OpenID Connect` provider supports.
    /// These values are described in Section 3.1.2.1 of `OpenID Connect Core 1.0` [OpenID.Core].
    pub(crate) display_values_supported: Option<Vec<String>>,

    /// OPTIONAL. JSON array containing a list of the Claim Types that the `OpenID Connect` provider
    /// supports.
    ...
    pub(crate) claim_types_supported: Option<Vec<String>>,

    /// RECOMMENDED. JSON array containing a list of the Claim Names of the Claims that the
    /// `OpenID Connect` provider MAY be able to supply values for.
    ...
    /// need to know when using the `OpenID Connect` provider.
    /// In particular, if the `OpenID Connect` provider does not support Dynamic Client Registration,
    /// then information on how to register Clients needs to be provided in this documentation.
    pub(crate) service_documentation: Option<String>,
    ...
    pub(crate) claims_locales_supported: Option<Vec<String>>,
    ...
    pub(crate) ui_locales_supported: Option<Vec<String>>,
    ...
    /// be pre-registered using the `request_uri`s registration parameter.
    /// Pre-registration is REQUIRED when the value is true.
    /// If omitted, the default value is false.
    pub(crate) require_request_uri_registration: Option<bool>,

    /// OPTIONAL. URL that the `OpenID Connect` provider provides to the person registering the
    /// Client to read about the OP's requirements on how the Relying Party can use the data
    /// provided by the OP.
    /// The registration process SHOULD display this URL to the person registering the Client if it
    /// is given.
    pub(crate) op_policy_uri: Option<String>,

    /// OPTIONAL. URL that the `OpenID Connect` provider provides to the person registering the
    /// Client to read about the `OpenID Connect` provider's terms of service.
    /// The registration process SHOULD display this URL to the person registering the Client if
    /// it is given.
    pub(crate) op_tos_uri: Option<String>,
}
